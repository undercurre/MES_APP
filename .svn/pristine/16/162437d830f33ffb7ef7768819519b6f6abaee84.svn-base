					...
					<view class="form-item">
						<u-table style="position: relative;height: 400rpx;overflow: auto;">
							<u-tr style="display: -webkit-box;position: sticky;top: 0;">
								<u-th style="width: 20%;flex: 0 0 20%;">序号</u-th>
								<u-th style="width: 20%;flex: 0 0 20%;">事项名称</u-th>
								<u-th style="width: 20%;flex: 0 0 20%;">事项描述</u-th>
								<u-th style="width: 20%;flex: 0 0 20%;">是否合格</u-th>
								<u-th style="width: 20%;flex: 0 0 20%;">备注</u-th>
							</u-tr>
							<u-tr  style="display: -webkit-box;" v-for="(item, index) in Maintain" :key="index">
								<u-td style="width: 20%;flex: 0 0 20%;overflow: hidden;line-height: 21px;">{{index+1}} </u-td>
								<u-td style="width: 20%;flex: 0 0 20%;overflow: hidden;line-height: 21px;">{{ item.ITEM_NAME }}</u-td>
								<u-td style="width: 20%;flex: 0 0 20%;overflow: hidden;line-height: 21px;">{{ item.REMARK }}</u-td>
								<u-td style="width: 20%;flex: 0 0 20%;overflow: hidden;line-height: 21px;display: flex;align-items: center"><u-switch v-model="item.ACTIVE" active-value="true" inactive-value="false" ></u-switch></u-td>
								<u-td style="width: 20%;flex: 0 0 20%;overflow: hidden;line-height: 21px;"><u-input v-model="item.DESC" type="text"/></u-td>
							</u-tr>
						</u-table>
					</view>
				</view>
