			...
			<view class="con-con" v-if="currentSubsection === 0">
				<u-table style="overflow-x: auto;" :style="{
					...
					<u-tr style="width: 1300rpx;" v-for="(item, index) in mainTable" :key="item.RESOURCE_NO" :class="{
						...
						<u-td style="width: 300rpx;overflow: hidden;line-height: 21px;flex: 2">{{ item.Warm_Time }}（{{ item.Warm_Time_H }}h）</u-td>
						<u-td style="width: 400rpx;overflow: hidden;line-height: 21px;flex: 3">{{ item.BEGIN_OPERATION_TIME }}</u-td>
						<u-td style="width: 300rpx;overflow: hidden;line-height: 21px;flex: 2">{{ item.RESOURCE_NO }}</u-td>
					</u-tr>
				</u-table>
			</view>
			<view class="con-con" v-if="currentSubsection === 1">
				<u-table style="overflow-x: auto;" :style="{
					...
					<u-tr style="width: 1700rpx;" v-for="(item, index) in mainTable1" :key="item.RESOURCE_NO" :class="{
						...
						<!-- <u-td style="width: 200rpx;overflow: hidden;line-height: 21px;flex: 1">{{ item.Bottle_NO }}</u-td> -->
						<u-td style="width: 300rpx;overflow: hidden;line-height: 21px;flex: 2">{{ item.LINE_NAME }}</u-td>
						<u-td style="width: 400rpx;overflow: hidden;line-height: 21px;flex: 3">{{ item.USED_Time }}（{{ item.USED_Time_H }}h）</u-td>
						<u-td style="width: 400rpx;overflow: hidden;line-height: 21px;flex: 3">{{ item.BEGIN_OPERATION_TIME }}</u-td>
						<u-td style="width: 400rpx;overflow: hidden;line-height: 21px;flex: 2">{{ item.RESOURCE_NO }}</u-td>
					</u-tr>
				</u-table>
			</view>
		</view>
